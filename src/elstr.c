...
/* elstr.c -- A C library for string manipulation
 *
...
static inline size_t str_sz_round(size_t sz) {
#define MINSIZE (1 << 4)
    size_t res = MINSIZE;
...
static void str_init_from_c_raw(str_ptr s, const char *sf, size_t len) {
    str_init_raw(s, len);
    memcpy(s->heap, sf, len + 1);
    s->length = len;
}

void str_init_from_c(str_ptr s, const char *sf) {
    size_t len = strlen(sf);
    str_init_from_c_raw(s, sf, len);
}

void str_init_from_str(str_ptr s, const str_t sf) {
...
void str_copy(str_t d, const str_t s) {
...
void str_copy_c(str_t d, const char *s) {
...
void str_copy_c_substr(str_t d, const char *s, int len) {
    len = (len >= 0 ? len : 0);
...
void str_append_c(str_t to, const char *from, int sep) {
...
void str_append(str_t to, const str_t from, int sep) {
...
void str_vprintf(str_t d, const char *fmt, int append, va_list ap) {
    ...
    /* The variable ns holds the number of characters needed for the printf
       call, not including the terminating null character. */
...
void str_printf(str_t d, const char *fmt, ...) {
    va_list ap;
    va_start(ap, fmt);
...
void str_printf_add(str_t d, const char *fmt, ...) {
    va_list ap;
    va_start(ap, fmt);
